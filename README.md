...
Statecraft needs no custom Marketplace action. After the CLI package is published, run the installed command as a normal job step and upload the complete `.statecraft` bundle with `if: always()` so a failed product-state check still leaves its report and screenshot evidence. The scan keeps its normal exit contract: `0` passes CI, `1` fails CI with product-state failures, and `2` fails CI for setup or configuration errors.

The [pre-publication GitHub Actions guide](docs/open-source/GITHUB_ACTIONS.md) includes the planned workflow, application readiness handling, a pinned report-upload action, and privacy/retention guidance. External installation remains blocked until the focused package-metadata release slice lands.
