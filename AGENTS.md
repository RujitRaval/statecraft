...
The approved [State Contract Guard roadmap](docs/designs/uiwitness-state-contract-guard.md) is active. T1 through T6 are complete with strict browser-independent contract parsing, RFC 8785 canonical digests, exhaustive contract comparison, deterministic verdict precedence, complete fresh-run CLI guard orchestration, exact-coordinate reproduction, deterministic machine verdicts, immutable content-addressed proposals, constrained metadata overlays, named single-use acceptance, one crash-recoverable generation transaction for report/evidence/verdict/proposal output, and the contract-first offline verdict experience; T7 through T14 remain separate future slices and must not be pulled forward implicitly.
