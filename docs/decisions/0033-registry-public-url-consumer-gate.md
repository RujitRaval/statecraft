...
- Give the job an independent 15-minute budget and provision browser system dependencies there. Retry only registry propagation, transient network failures, and install timeouts within one bounded three-minute elapsed-time window. Permanent npm and process errors fail immediately.
