...
## Context
The Public URL Quick Check journey spans npm installation, browser provisioning, bounded discovery, persisted evidence, overwrite-safe project promotion, generated module imports, configured scanning, and report opening. Workspace and packed-tarball tests prove candidate artifacts, but neither proves that a newly published consumer can complete the launch journey from the public registry.
...
- Serve a deterministic two-page loopback fixture owned by the gate. Run evidence-only `check`, execute its exact printed `check --write-config` command, run the untouched generated `scan`, then open the installed report with the CLI binary.
- Require the exact eight-cell route × public-state × viewport × theme coordinates, all-pass summaries, non-empty in-boundary screenshots, schema-v1 JSON, kinetic offline HTML, unchanged generated source after scan, and a successful report open.
