...
## Decision
- Add a dependent post-publication release job that accepts the exact GitHub Release tag and installs `statecraft-ui` plus the runner manifest's exact Playwright version into an empty CommonJS-default `npm init -y` project from the explicit npmjs registry.
- Give the job an independent 15-minute budget and provision browser system dependencies there. Retry only bounded registry propagation, transient network failures, and install timeouts with per-command limits that fit that ceiling. Permanent npm and process errors fail immediately.
