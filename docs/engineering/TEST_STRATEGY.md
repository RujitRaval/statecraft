...
Package-release tests validate the repository-to-npm version mapping, reject nonzero fourth components and metadata drift, and synchronize isolated fixture manifests. The package smoke packs all four public workspaces, runs npm publication dry runs, installs only the resulting tarballs into an isolated consumer, checks the shipped file allowlist and exact internal dependency versions, imports every public API, invokes the installed command shim for CLI help, and verifies generated initialization content. Publisher tests cover unsafe, incomplete, symbolic-link, and non-file inputs; dependency and CLI-last order; serialized monotonic `latest` publication; new packages; byte-identical partial-release retries; dist-tag inconsistency; mismatched registry integrity; malformed and non-404 registry failures; bounded child processes; and the absence of shell command construction.
