...
Report transformation tests verify deterministic first-seen column and route/state ordering, aligned missing cells, report-relative screenshot references, and rejection of invalid schema input. Renderer tests cover summary/matrix/detail content, empty selections, HTML escaping for report-controlled diagnostics, all five filter contracts, no external assets, and an exact embedded-script CSP hash. A real Chromium `file://` test covers AND filtering across route, state, viewport, theme, and status; no-match/reset behavior; URL restoration; filtered table alignment; keyboard detail opening and Escape return; responsive layout; 44-pixel mobile controls; screenshots; and zero network requests. Runner persistence tests cover coordinated replacement, owner-private file modes, artifact/JSON/HTML symbolic-link boundaries, stable errors, and rollback; report package-boundary tests cover the built renderer API. Browser-backed CLI scan tests prove PNG, JSON, and HTML are produced together and that terminal output points to the offline document.
