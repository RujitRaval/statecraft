...
The optional `contractVerdict` input is additive. The renderer validates schema version, completeness, status, digests, finding identity/order, expected and actual outcomes, commands, and exception display metadata before rendering. A guard report leads with contract status, matched/promised counts, and complete non-match findings, then preserves the existing execution tape, filters, matrix, and inspector. Ordinary schema-v1 report consumers and one-argument calls remain unchanged.
