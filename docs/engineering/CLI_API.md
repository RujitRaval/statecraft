...
`@statecraft/cli` exposes deterministic config loading plus executable `init` and `scan` workflows. Scan composes the existing core planner and Playwright persistence contracts; it does not generate HTML or implement `open`.
...
Missing commands, unsupported commands (including the deferred `open` command), extra `init` arguments, malformed scan options, config errors, unknown route IDs, and run-level failures return `2`. Help and all-pass scans return `0`. A completed scan containing failed cells returns `1` after persisting its report.

## Programmatic command, init, and scan API

```ts
import { initProject, runCli, scanProject } from "@statecraft/cli";
