...
The persisted runner prevalidates individual cells and cross-cell report invariants, translates every settled capture into the schema-v1 core `ExecutionResult` contract, including failures that retain a successfully captured screenshot, and calculates summary coverage from the configured cells. It validates the complete `UIWitnessReport`, delegates pure HTML rendering to `uiwitness-report`, and invokes an optional browser-neutral generation finalizer before persistence. A process-owned, phase-aware run lock covers capture and publication. Writes stage screenshots, JSON, HTML, and validated sidecars with a durable recovery journal; reject symbolic links, hard links, unsafe/reserved paths, and changed immutable members; normalize private modes; fsync files/directories; and swap the generation with rollback bookkeeping. HTML appears last among report content and the stable committed-generation marker appears last overall. Failed publication restores the previous set. An abandoned journaled publication rolls back before that marker swap and keeps the coherent new set after it; ambiguous or incomplete recovery preserves staging and lock state. The runner does not expose CLI behavior.
...
## Contracts
Use deterministic `.uiwitness/` artifact paths. Metadata is separate from filenames. JSON starts with `schemaVersion: 1` and is an external contract.
