...
After publication, a dependent release job gives the registry-only public URL gate an independent 15-minute budget. It creates a CommonJS npm project in a private temporary directory, accepting both npm's implicit CommonJS manifest and npm 11's explicit `"type": "commonjs"` form, installs the exact release plus the runner manifest's exact Playwright version from the explicit npmjs registry, and provisions the browser's Linux dependencies inside that job. Against a deterministic two-page loopback fixture, it runs evidence-only `check`, executes the exact printed `check --write-config` command, and runs the untouched generated `scan`. Exact eight-cell coordinates, screenshots, schema-v1 JSON, kinetic HTML, generated imports, and source-file stability must pass. A bounded three-minute elapsed-time window covers registry propagation and command timeouts only; the temporary consumer is always removed.
