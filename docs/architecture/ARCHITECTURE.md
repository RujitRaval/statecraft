...
The State Contract Guard orchestration slices add process-based `guard`, exact-coordinate `scan`, and `contract init|inspect|annotate|accept`. Guard anchors all config, contract, scenario, verdict, source-generation, proposal, and metadata paths to the canonical invocation workspace, rejects symbolic links, prevalidates inputs before browser launch, executes the complete unfiltered matrix, and compares only that run's in-memory schema-v1 report through `uiwitness-core`. The adapter owns the exhaustive v1 config fingerprint and semantic run digest projections. A deterministic private machine verdict carries stable `0`/`1`/`2` process meaning and shell-safe reproduction/remediation commands without diagnostic messages.

Core owns canonical immutable proposal sources, named operations, constrained metadata overlays, digest verification, and selected-change application. The CLI owns content-addressed filenames, safe filesystem boundaries, a single contract-writer lock, current config/contract revalidation, safe contract replacement or initial no-clobber creation, and single-use proposal consumption. The mutable overlay is separate from proposal bytes so review metadata cannot change proposal identity. The CLI surface remains process-only. The existing runner transaction still owns PNG/JSON/HTML publication; the approved later generation-transaction slice will atomically include the verdict and proposal family and close cross-output crash windows.
