...
Follow `AGENTS.md`, `DESIGN.md`, and the specification files under `codex/` and `docs/`. Phase 1 through Phase 7 and the approved Public URL Quick Check roadmap are complete, including `uiwitness check <url>`, overwrite-safe `--write-config` promotion, public launch guidance, and the registry-only check → promotion → scan → open release gate. Keep future work in an explicitly approved roadmap slice.
