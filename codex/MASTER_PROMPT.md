...
The approved [State Contract Guard roadmap](../docs/designs/uiwitness-state-contract-guard.md) is active. T1 through T7 are complete: strict contracts and canonical digests, deterministic comparison and verdict precedence, complete fresh-run `uiwitness guard` orchestration, exact-coordinate reproduction, deterministic machine verdicts, immutable content-addressed proposals, constrained metadata overlays, named single-use acceptance, one crash-recoverable generation transaction for report/evidence/verdict/proposal output, the contract-first offline verdict experience, and a thin full-SHA-pinned GitHub Action with exact project-local CLI parity, bounded summaries/annotations, and opt-in evidence upload. T8 through T14 remain separate approved slices and must not be pulled forward implicitly.
