...
The approved [State Contract Guard roadmap](../docs/designs/uiwitness-state-contract-guard.md) is active. T1 through T5 are complete: strict contracts and canonical digests, deterministic comparison and verdict precedence, complete fresh-run `uiwitness guard` orchestration, exact-coordinate reproduction, deterministic machine verdicts, immutable content-addressed proposals, constrained metadata overlays, named single-use acceptance, and one crash-recoverable generation transaction for report/evidence/verdict/proposal output. T6 through T14 remain separate approved slices and must not be pulled forward implicitly.
