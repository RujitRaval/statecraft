# statecraft-ui

The npm package for the `statecraft` CLI. Statecraft renders configured UI product states with Playwright, captures screenshots and diagnostics, and writes an offline coverage report.

```bash
npm install --save-dev statecraft-ui playwright@1.62.1
npx playwright install chromium
npx statecraft init
npx statecraft scan
```

The package also exports `defineConfig`, config discovery/loading, initialization, scan orchestration, and report-opening APIs for TypeScript consumers.

See the [Statecraft repository](https://github.com/RujitRaval/statecraft) for configuration, scenario, CI, privacy, and contribution guidance.
