      ...
      (route) => `<tbody data-route-group="${escapeHtml(route.id)}">${route.rows
        ...
        (row, rowIndex) => `<tr data-matrix-row data-route="${escapeHtml(row.routeId)}" data-state="${escapeHtml(row.stateId)}">
          ...
          ${row.cells.map((cell, index) => {
            ...
            return `<td data-matrix-slot data-viewport="${escapeHtml(column.viewportId)}" data-theme="${escapeHtml(column.theme)}" data-status="${cell?.execution.status ?? "missing"}">${matrixCell(cell, column, row.routeId, row.stateId)}<span class="filtered-cell" hidden aria-hidden="true">—</span></td>`;
          }).join("")}
        </tr>`,
        )
        .join("")}</tbody>`,
...
@media(max-width:700px){.shell{width:min(100% - 20px,1600px);padding-top:28px}.hero h1{font-size:3rem}.summary{grid-template-columns:repeat(2,1fr)}.filters,.matrix-panel,.detail{padding:14px}.filters-heading,.detail-heading{align-items:start;flex-direction:column}.filters-heading{gap:8px}#report-filters{grid-template-columns:1fr}.reset-filters{width:100%}.run-meta{display:grid;gap:6px}.metric{padding:14px}}
